...
| Section 4: contexts, buffers, programs, queues, submissions, and events | `virtio-accel-core`, construction-time metadata validation, object-table lifecycle tests, deterministic mock execution with verifiable buffer output, and full serialized split-queue scenarios | The reusable backend suite is #24 |
...
| Sections 7-9: ownership, time, progress, and error truth | Per-method provider contracts plus generational-ID, timeout, mock lifecycle, deterministic fault injection at every provider boundary, ownership auditing, indeterminate-submit, end-to-end recovery, and response-atomicity tests | The reusable conformance suite is #24 |
...
| `wire-abi.md` section 7: preflight and response atomicity | Error-shape vectors, ownership-aware core error types, explicit transfer-failure contracts, end-to-end short-completion rejection, and scripted pre/post-mutation faults | The reusable conformance suite is #24 |
