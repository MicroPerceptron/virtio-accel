...
| Section 5: mandatory baseline, reserved features, flags, and scalar namespaces | Primary ABI namespace tests plus clean-room request validation, edge vectors, and the release-policy invariant check | Optional features remain unadvertised until a later protocol version assigns negotiation and conformance rules |
| Section 6: versions, exact lengths, unknown values, and extension rules | Config/version edge vectors, both codecs, immutable manifests, layout assertions, release policy, and freeze audit | Protocol 1.0 assigned bytes are frozen by `freeze-audit.md` |
| Sections 7-9: ownership, time, progress, and error truth | Per-method provider contracts plus the standard backend suite, broken-backend controls, generational-ID, timeout, deterministic fault injection, ownership accounting, copy-path diagnostics, indeterminate-submit, end-to-end recovery, response-atomicity tests, and deterministic performance-budget evidence | Hardware-provider timing baselines remain adapter-specific release evidence |
...
| `wire-abi.md` sections 8-9: versioned artifacts and change control | Checked-in inputs, drift tests, coordinated-change documentation, release policy, and freeze audit | Post-freeze incompatible changes require a new protocol-major directory |
