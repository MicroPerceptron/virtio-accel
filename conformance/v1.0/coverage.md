...
| Section 4: contexts, buffers, programs, queues, submissions, and events | `virtio-accel-core`, the reusable transport-free backend suite, construction-time metadata validation, object-table lifecycle tests, deterministic mock execution with verifiable buffer output, and full serialized split-queue scenarios | Provider-specific artifact fixtures remain implementation inputs |
