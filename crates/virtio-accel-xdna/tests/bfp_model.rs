//! Bit-level reference model of XDNA2 `bfp16ebs8` — verbatim copy of
//! `research/bfp16ebs8/runner/src/model.rs` (issue #146, PR #161), vendored per the issue
//! #148 requirement to reuse the characterization model exactly. A path dev-dependency on the
//! non-workspace research project would leak into the published manifest, so the file is
//! copied with this provenance header; keep the two in sync by re-copying, never by editing
//! here. Its tests replay recorded silicon planes, so this target also pins the model on
//! hosts without an NPU.
#![allow(dead_code)] // Two includers (this target and the va_xdna experiment test) use subsets.
